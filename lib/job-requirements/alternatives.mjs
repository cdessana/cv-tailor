...
export function validateAlternativeSemantics(group) {
  const quote = group.context.toLocaleLowerCase();
  const hasSignal = choiceSignal.test(group.context);
  const containsAllValues = group.values.every(v => quote.includes(v.toLocaleLowerCase()));
  
  if (!hasSignal && !containsAllValues) {
