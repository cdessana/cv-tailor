...
export async function diagnoseEnvironment({
  ...
  checks.push(await commandChecks.npm);
  ...
  checks.push(await commandChecks.pdftotext, await commandChecks.pdfinfo);
  ...
  checks.push(
    env.GEMINI_API_KEY
      ? result(
          ...
          "optional",
          "pass",
          "GEMINI_API_KEY is present; its value was not read or displayed."
        )
      : result(
          "gemini",
          "Gemini",
          "optional",
          "warn",
          "GEMINI_API_KEY is not set; cloud job parsing is disabled.",
          "Set GEMINI_API_KEY only if you explicitly choose Gemini."
        )
  );
