...
export function isGenericSectionLead(unit) {
  ...
  return /^(?:we(?:'re| are) looking for someone who meets the minimum requirements to be considered for the role|the ideal candidate(?: will| should)?|candidates? should meet the following requirements)\.?$/iu.test(
    normalize(unit.text)
  );
}
...
export function isResponsibilityLead(unit) {
  if (unit.type === "bullet") return false;
  return /^(?:como\s+.+,?\s+)?você\s+atuará\s+.+\s+e\s+suas\s+principais\s+atividades\s+serão:?$/iu.test(
    normalize(unit.text)
  );
}
