...
export function validateExampleCoverage(item) {
  ...
  const errors = [];
  // A short qualifier such as "primarily AWS" narrows the requirement itself;
  // it is not disposable example metadata. Require the model to retain that
  // source wording in value so downstream matching can see the technology.
  const value = normalize(item.value);
  const qualifier = new RegExp(`^${value.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&")}(?:,|\\s)+(?:mainly|primarily|especially|particularly|principalmente|especialmente|sobretudo)\\s+([^.;:!?]+)`, "iu");
  const match = quote.trim().match(qualifier);
  if (match) {
    errors.push({
      code: "missing_illustrative_qualifier", path: "/value", quote,
      message: "A short technology qualifier attached to the qualification was omitted from its value.",
      instruction: "Copy the complete qualification including the primarily/mainly qualifier. It may also retain examples, but do not drop the named technology from value.",
    });
  }
