...
const schema = JSON.parse(
  await fs.readFile(
    new URL("../../schemas/job.schema.json", import.meta.url),
    "utf8"
  )
);
const validateJob = new Ajv({
  strict: true,
  allErrors: true,
  coerceTypes: false,
...
const normalized = (value) =>
  String(value).replace(/\s+/gu, " ").trim().toLocaleLowerCase();
...
function shadowedItemIndices(items) {
  ...
  items.forEach((item, index) => {
    if (
      item.type !== "item" ||
      !["required", "preferred"].includes(item.classification)
    )
      return;
...
export function mapToJob(extraction) {
  ...
  for (const [key, record] of Object.entries(metadata)) {
    const distinct = new Set(
      (record.candidates ?? []).map((candidate) =>
        candidate.value.replace(/\s+/gu, " ").trim().toLocaleLowerCase()
      )
    );
    if (distinct.size > 1)
      warnings.push(
        issue(
          ...
          `Human validation required for ${key}. Selected "${record.value}"; other source-backed candidates remain.`,
          {
            ...
            selected: { value: record.value, evidence: record.evidence },
  ...
  extraction.items.forEach((item, index) => {
    ...
    if (item.type === "alternative") {
    ...
    } else if (item.kind === "requirement" && item.classification === "ambiguous") {
      // The final contract already has a neutral qualifications field. Keep
      // source-backed prose there when enrichment can identify it as a
      // qualification but cannot truthfully assign required/preferred strength.
      qualifications.push(itemValue);
    } else if (
      item.kind === "ambiguous" ||
      item.classification === "ambiguous"
    ) {
      if (item.kind === "skill") skills.push({ name: itemValue, keywords: [] });
      else if (item.kind === "competency") requirements.competencies.push(itemValue);
      else
        errors.push(
          issue(
            "ambiguous_item",
            path,
            "Ambiguous items cannot be safely projected into the legacy job format.",
            { value: itemValue }
          )
        );
    } else {
      errors.push(
        issue(
          ...
          path,
          "Item classification has no legacy mapping.",
          { value: itemValue }
        )
      );
    }
    ...
    if (
      ...
      const classification =
        item.kind === "competency" ? "competencies" : item.classification;
      if (requirements[classification]?.includes(itemValue)) {
        requirementExamples.push({
          ...
          values: [
            ...new Set(
              allExamples.map((v) => v.trim()).filter((v) => v.length > 1)
            ),
          ],
        });
      }
  ...
  if (
    ...
    requirements.competencies.length
  )
    job.requirements = requirements;
  if (alternativeRequirements.length)
    job.alternativeRequirements = alternativeRequirements;
  if (responsibilities.length) job.responsibilities = responsibilities;
  if (skills.length) job.skills = skills;
  if (qualifications.length) job.qualifications = qualifications;
    ...
    return {
      ...
      errors: [
        issue(
          "final_schema",
          "/",
          "Mapped job failed final schema validation.",
          { details: structuredClone(validateJob.errors) }
        ),
      ],
      warnings,
    };
