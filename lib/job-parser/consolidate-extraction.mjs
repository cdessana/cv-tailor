...
function key(item) {
  const identity = item.type === "alternative"
    ...
    : text(item.value);
  return `${item.type}\u0000${item.kind}\u0000${item.classification}\u0000${item.provenance ?? ""}\u0000${identity}\u0000${text(item.evidence.quote)}`;
}
...
export function consolidateExtraction(document, extraction) {
  ...
  result.items = items;
  if (providerReport && !Object.getOwnPropertyDescriptor(result, "providerReport")) {
