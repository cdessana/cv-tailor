...
function safeEmptyExclusion(block) {
  ...
  if (/^(?:about the team|about the job|how to apply|apply now|benefits|perks|sobre a vaga|benefícios|candidate-se)[:.!?]?$/iu.test(text)) {
...
export function normalizeEmptyBlocks(response, blocks, onExclusion = () => {}) {
  for (const block of blocks) {
    ...
    if (result?.status === "extracted" && !result.items.length && !result.alternatives.length && !Object.keys(result.metadata ?? {}).length) {
      ...
      if (reason) {
        result.status = "excluded"; result.reason = reason;
        onExclusion({ code: "empty_block_excluded", blockId: block.id, reason });
      }
    }
  }
  return response;
...
const alternativeKeys = new Set(["type", "operator", "kind", "classification", "values", "evidence", "sourceSection"]);
...
export function canonicalizeBlockRecords(response, onChange = () => {}) {
  for (const [blockId, result] of Object.entries(response.blocks ?? {})) {
    if (!Array.isArray(result.items) || !Array.isArray(result.alternatives)) continue;
    const ordinary = [];
    for (const record of result.items) {
      if (record?.type === "alternative" && Object.keys(record).every(key => alternativeKeys.has(key))
        && record.operator === "anyOf" && Array.isArray(record.values) && record.values.length >= 2) {
        result.alternatives.push(record);
        onChange({ code: "misplaced_alternative_canonicalized", blockId,
          reason: "Moved a complete anyOf record from items to alternatives without changing values or evidence." });
      } else {
        const canonical = canonicalizeDirectAlternative(record);
        if (canonical?.type === "alternative") {
          result.alternatives.push(canonical);
          onChange({ code: "direct_alternative_canonicalized", blockId,
            reason: "Converted a standalone direct choice to anyOf without changing evidence." });
...
export function createBlockContract(document) {
  ...
  const shapeErrors = response => validate(response) ? [] : structuredClone(validate.errors);
  ...
  function reconcile(response, onChange = () => {}) {
    ...
    if (errors.length) throw new TypeError(`Invalid block response: ${JSON.stringify(errors)}`);
    for (const block of blocks) {
      const result = response.blocks[block.id];
      for (const record of [...result.items, ...result.alternatives, ...Object.values(result.metadata)]) {
        if (!normalized(block.text).includes(normalized(record.evidence.quote))) continue;
        if (record.values) {
          record.values = record.values.map(value => restoreSourceCase(value, record.evidence.quote));
        } else record.value = restoreSourceCase(record.value, record.evidence.quote);
        for (const example of record.examples ?? []) {
          example.value = restoreSourceCase(example.value, record.evidence.quote);
        }
      }
    }
    ...
    const targets = blocks.filter(block => {
      ...
      if (result.status !== "excluded" || !(result.items.length || result.alternatives.length || Object.keys(result.metadata).length)) return false;
    ...
    for (const block of targets) {
      if (feedback.some(error => !error.blockId || error.blockId === block.id)) continue;
      const originalReason = response.blocks[block.id].reason;
      response.blocks[block.id] = proposed.blocks[block.id];
      onChange({ code: "block_status_corrected", blockId: block.id, from: "excluded", to: "extracted", originalReason,
        reason: "Retained all schema-valid, source-backed records despite the contradictory excluded status." });
    }
    return response;
  }
  ...
  function inspect(response) {
    ...
    for (const block of blocks) {
      ...
      for (const entry of records) {
        ...
        if (!key) {
          for (const error of validateItemSemantics(record, { metadata: result.metadata })) {
            errors.push({ ...details, ...error, path: entry.path + error.path });
          }
        }
        for (const error of validateEvidence(document, extraction).errors) {
          ...
          errors.push({ ...details, ...error, path: entry.path + error.path.slice(prefix.length),
            ...(key === "workArrangement" ? { instruction: "Copy the complete source-backed work arrangement, preserving hashtags such as #remotefirst and conditions such as sempre que a função permitir. Do not strip punctuation or infer unrestricted remote work." } : {}),
          });
        }
      }
    }
  ...
  function assemble(response, logger = {}) {
    ...
    for (const block of blocks) {
      ...
      for (const [key, record] of Object.entries(result.metadata ?? {})) {
        ...
        extraction.metadata[key] = mergeMetadataRecord(extraction.metadata[key], attached, key);
      }
    }
    ...
    for (const block of blocks) {
      ...
      if (result && result.status === "extracted" && !result.items.length && !result.alternatives.length && Object.keys(result.metadata ?? {}).length
        ...
        repeatedMetadata.set(block.id, { unitId: block.id, status: "excluded", reason: "Source-backed metadata retained in the candidate collection; primary value retained from another block." });
      }
    }
  ...
  return { blocks, schema, toolSchema, assemble, inspect, shapeErrors, reconcile };
}
