  ...
  function inspect(response) {
    ...
    for (const block of blocks) {
      ...
      if (
        ...
        !isGenericSectionLead(block) &&
        !isCompanyWorkPolicy(block) &&
        !isEmployerPolicyContext(block) &&
        !isResponsibilityLead(block)
      ) {
        errors.push({
          blockId: block.id,
          path,
          ...
          instruction:
            "Extract the complete source-backed qualification, responsibility, or competency. Do not exclude substantive content under this heading.",
        });
      }
