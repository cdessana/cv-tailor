...
function promptFor(blocks, contextText) {
  return [
    ...
    'Metadata also supports workArrangement, using the same value and evidence.quote structure. EmploymentType describes full-time, part-time or contract. WorkArrangement describes remote, hybrid or onsite work; copy the original phrase, e.g. value "work 100% remotely" from quote "work 100% remotely with flexible hours". Keep geography in location. Inspect benefits/perks for workArrangement before excluding them. Omit absent metadata, never infer it.',
    ...
    for (let offset = 0; offset < ids.length; offset += 3) {
      ...
      try {
        ...
        for (correctionAttempt = 0; correctionAttempt < 2; correctionAttempt++) {
          raw = undefined;
          try {
            const accepted = await request({ ...document, correction });
