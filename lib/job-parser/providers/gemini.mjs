...
import { canonicalizeBlockRecords, createBlockContract, normalizeEmptyBlocks } from "./gemini-blocks.mjs";
...
function scopedPreviousResponse(error, blockIds) {
  ...
  const selected = Object.fromEntries(blockIds
...
function createBatchRequest({
  ...
  return async (input) => {
    ...
    const contents = [{ role: "user", parts: [{ text: promptFor(blockContract.blocks, input.contextText ?? input.originalText, input.confirmedMetadata) }] }];
    ...
    const blocksResponse = decodeGeminiResponse(payload, blockContract.blocks.map(block => block.id));
    let extraction;

    // Now validate and assemble as before
    try {
      canonicalizeBlockRecords(blocksResponse, warning => logger.warn?.(`[job-parser] ${JSON.stringify(warning)}`));
      const shapeErrors = blockContract.shapeErrors(blocksResponse);
      if (shapeErrors.length) throw new TypeError(`Invalid block response: ${JSON.stringify(shapeErrors)}`);
      blockContract.reconcile(blocksResponse, warning => logger.warn?.(`[job-parser] ${JSON.stringify(warning)}`));
      normalizeEmptyBlocks(blocksResponse, blockContract.blocks, warning => logger.warn?.(`[job-parser] ${JSON.stringify(warning)}`));
      if (debug) logger.debug?.(`[job-parser] Extraction after normalization: ${JSON.stringify(blocksResponse)}`);
      const feedback = blockContract.inspect(blocksResponse);
      if (debug) logger.debug?.(`[job-parser] Inspection feedback: ${JSON.stringify(feedback)}`);
      if (feedback.length) {
        const failure = providerError("GEMINI_SCHEMA_ERROR", `Gemini output failed intermediate schema validation: Block validation failed: ${JSON.stringify(feedback)}`);
        failure.invalidBlockIds = [...new Set(feedback.map(error => error.blockId).filter(Boolean))];
        failure.blocksResponse = structuredClone(blocksResponse);
        throw failure;
      }
      
      extraction = blockContract.assemble(blocksResponse, debug ? logger : {});
      assertExtraction(extraction);
    } catch (error) {
      const detail = error instanceof Error ? error.message : String(error);
      const compactDetail = detail.length > 2400 ? `${detail.slice(0, 2400)}…` : detail;
      logger.error?.(`[job-parser] Gemini output failed validation.${debug ? ` ${compactDetail}` : ""}`);
      const failure = providerError("GEMINI_SCHEMA_ERROR", `Gemini output failed intermediate schema validation: ${compactDetail}`, error);
      if (Array.isArray(error.invalidBlockIds)) failure.invalidBlockIds = error.invalidBlockIds;
...
export function createGeminiProvider(options = {}) {
  ...
  return async input => {
    ...
    const batchSize = positiveInteger(options.batchSize ?? process.env.GEMINI_BATCH_SIZE, DEFAULT_BATCH_SIZE);
    const maxCorrections = positiveInteger(options.maxCorrections ?? process.env.GEMINI_MAX_CORRECTIONS, DEFAULT_MAX_CORRECTIONS, 3);
    ...
    for (let offset = 0; offset < ids.length; offset += batchSize) {
      ...
      const request = createBatchRequest({ ...options, onRawResponse: async text => {
        raw = text;
        responses.push({ batch, attempt: correctionAttempt + 1, blockIds: responseBlockIds, response: text });
      ...
      try {
        ...
        let acceptedBlocks;
        for (correctionAttempt = 0; correctionAttempt <= maxCorrections; correctionAttempt++) {
          raw = undefined;
          try {
            const accepted = await request({ ...correctionDocument, correction });
            ...
            acceptedBlocks = acceptedBlocks ? { ...acceptedBlocks, ...accepted.blocks } : accepted.blocks;
            // A targeted correction must still make the original batch valid as
            // a whole before its approved blocks are retained.
            const batchExtraction = createBlockContract(document).assemble({ blocks: acceptedBlocks });
            const batchEvidence = validateEvidence(document, batchExtraction);
            if (!batchEvidence.valid) throw providerError("GEMINI_EVIDENCE_ERROR", JSON.stringify(batchEvidence.errors));
            break;
          } catch (error) {
            const correctable = raw !== undefined && ["GEMINI_SCHEMA_ERROR", "GEMINI_RESPONSE_ERROR", "GEMINI_EVIDENCE_ERROR"].includes(error.code);
            if (correctionAttempt >= maxCorrections || !correctable) throw error;
            const responseSet = new Set(responseBlockIds);
            const invalidIds = error.invalidBlockIds?.filter(id => responseSet.has(id)) ?? [];
            const targeted = invalidIds.length > 0 && invalidIds.length < responseBlockIds.length && error.blocksResponse;
            if (targeted) {
              const currentValidBlocks = Object.fromEntries(responseBlockIds.filter(id => !invalidIds.includes(id))
                .map(id => [id, structuredClone(error.blocksResponse.blocks[id])]));
              acceptedBlocks = { ...(acceptedBlocks ?? {}), ...currentValidBlocks };
              const invalidSet = new Set(invalidIds);
              const sections = document.sections.map(section => ({ ...section,
                units: section.units.filter(unit => invalidSet.has(unit.id)),
              })).filter(section => section.units.length);
              correctionDocument = { ...document, sections, contextText: targetContext(sections) };
              responseBlockIds = invalidIds;
              logger.warn?.(`[job-parser] Gemini batch ${batch}/${total} has ${invalidIds.length} invalid block(s); requesting targeted correction.`);
            }
            const previousResponse = scopedPreviousResponse(error, responseBlockIds);
            correction = {
              ...(previousResponse ? { previousResponse } : {}),
              validationError: error.cause?.message ?? error.message,
              sourceBlocks: correctionDocument.sections.flatMap(section => section.units.map(unit => ({ id: unit.id, text: unit.originalText }))),
            };
            if (!targeted) logger.warn?.(`[job-parser] Gemini batch ${batch}/${total} failed validation; requesting correction ${correctionAttempt + 1}/${maxCorrections}.`);
          }
        }
        ...
        const processedExtraction = createBlockContract(processedDocument).assemble(combined);
        confirmedMetadata = processedExtraction.metadata ?? confirmedMetadata;
