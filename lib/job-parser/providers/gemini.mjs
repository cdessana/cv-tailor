      ...
      try {
        ...
        for (correctionAttempt = 0; correctionAttempt < 2; correctionAttempt++) {
          ...
          try {
            ...
            const invalidIds = error.invalidBlockIds?.filter(id => targetSet.has(id)) ?? [];
            const targeted = invalidIds.length > 0 && invalidIds.length < targetIds.length && error.blocksResponse;
            if (targeted) {
              acceptedBlocks = Object.fromEntries(targetIds.filter(id => !invalidIds.includes(id))
            ...
            if (!targeted) logger.warn?.(`[job-parser] Gemini batch ${batch}/${total} failed validation; requesting one correction.`);
          }
        }
