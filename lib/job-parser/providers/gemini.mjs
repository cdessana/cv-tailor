...
function promptFor(blocks, contextText, confirmedMetadata = {}) {
  return [
    ...
    "Within a block, extract granular, atomic skills, technologies, and concepts. Do not extract full paragraphs or long sentences as the value. Instead, identify the core requirement (e.g., extract 'programming languages' instead of 'Proficiency in multiple programming languages'). When a sentence contains an AND list of independent skills (e.g., 'English and Portuguese' or 'LLMs, automation, and design tools'), split them into separate, independent item records. Both items must share the same full sentence in their evidence.quote. The extracted value must still be a verbatim contiguous substring of the source text to pass evidence validation. Explicitly extract professional domains (e.g., 'Large-scale applications', 'SaaS', 'Cloud-native', 'High-performance systems') as independent granular items. Preserve every distinct duration, scope and condition.",
...
function createGeminiDecisionProvider(options = {}) {
  const apiKey = options.apiKey ?? process.env.GEMINI_API_KEY;
  const model = options.model ?? process.env.GEMINI_MODEL ?? DEFAULT_MODEL;
  const endpoint = options.endpoint ?? DEFAULT_ENDPOINT;
  const fetchImpl = options.fetchImpl ?? globalThis.fetch;
  const timeoutMs = options.timeoutMs ?? envNumber("GEMINI_TIMEOUT_MS", 120000);
  const maxAttempts = positiveInteger(options.maxAttempts ?? process.env.GEMINI_MAX_ATTEMPTS, DEFAULT_MAX_ATTEMPTS, 10);
    ...
    properties: {
      decisions: {
        ...
        items: {
          ...
          properties: {
            unitId: { type: "string" },
            action: { type: "string", enum: ["exclude", "unresolved", "requirement", "responsibility", "alternative", "metadata"] },
            values: { type: "array", items: { type: "string" } },
            metadataKey: { type: "string", enum: ["company", "title", "location", "workArrangement", "employmentType", "sourceUrl"] },
            value: { type: "string" },
            reason: { type: "string" },
          },
        },
  ...
  return async (input) => {
    const blocks = input.sections.flatMap((section) => section.units.map((unit) => ({
      id: unit.id, type: unit.type, heading: section.heading?.text ?? null,
    ...
    const request = {
      ...
      body: JSON.stringify({
          contents: [{ role: "user", parts: [{ text: [
            "Classify only these ambiguous source blocks. Never reproduce evidence, metadata, headings, classifications, or coverage.",
            "Return decide_units. Use unresolved with a concise reason when qualification versus context cannot be determined. Use requirement for a source-backed candidate qualification: explicit required/preferred headings retain their strength, while neutral or Activities prose becomes a generic qualification. alternative requires a clear OR/OU and exact values; metadata requires an explicit metadataKey and exact source value; otherwise exclude context.",
            `SOURCE BLOCKS: ${JSON.stringify(blocks)}`,
          ].join("\n\n") }] }],
          tools: [{ functionDeclarations: [{ name: "decide_units", parameters }] }],
          toolConfig: { functionCallingConfig: { mode: "ANY", allowedFunctionNames: ["decide_units"] } },
      }),
    };
    let response;
    let lastError;
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      try {
        ...
        lastError = providerError("GEMINI_REQUEST_ERROR", `Gemini returned HTTP ${response.status}.`);
      } catch (error) {
        lastError = providerError(controller.signal.aborted ? "GEMINI_TIMEOUT" : "GEMINI_REQUEST_ERROR", "Gemini decision request failed.", error);
      } finally {
        clearTimeout(timer);
      }
      if (attempt < maxAttempts) continue;
    }
    if (!response || (!response.ok && lastError)) throw lastError;
    if (!response.ok) throw providerError("GEMINI_REQUEST_ERROR", `Gemini returned HTTP ${response.status}.`);
    const payload = await response.json();
    const call = payload?.candidates?.[0]?.content?.parts?.find((part) => part.functionCall)?.functionCall;
    if (call?.name !== "decide_units" || !Array.isArray(call.args?.decisions))
      throw providerError("GEMINI_RESPONSE_ERROR", "Gemini decision response requires decide_units decisions.");
    const ids = new Set(blocks.map((block) => block.id));
    const seen = new Set();
    for (const decision of call.args.decisions) {
      if (!ids.has(decision?.unitId) || seen.has(decision.unitId))
        throw providerError("GEMINI_RESPONSE_ERROR", "Gemini decision response has unknown or duplicate source IDs.");
      seen.add(decision.unitId);
    }
    return { decisions: structuredClone(call.args.decisions) };
  };
}
...
export function createGeminiProvider(options = {}) {
  if (options.decisionMode) {
    const decide = createGeminiDecisionProvider(options);
    return async (input) => {
      ...
      const decisions = [];
      const stats = { started: 0, completed: 0, retries: 0, timeouts: 0, checkpoint: { enabled: false, resumed: false, cleared: false } };
      for (const batch of plan.batches) {
        stats.started += 1;
        const result = await decide({ ...input, sections: batch.sections });
        decisions.push(...result.decisions);
        stats.completed += 1;
      }
      const result = { decisions };
      Object.defineProperty(result, "providerReport", { value: stats, enumerable: false });
      return result;
    };
  }
  ...
  return async (input) => {
    ...
    const batchStats = {
      ...
      timeouts: 0,
      splits: 0,
      checkpoint: { enabled: false, resumed: false, cleared: false },
    };
    ...
    for (const batchInfo of plan.batches) {
      ...
      const request = createBatchRequest({
        ...options,
        onRequestRetry: () => { batchStats.retries += 1; },
        onRequestTimeout: () => { batchStats.timeouts += 1; },
