...
function createBatchRequest({
  ...
  return async (input) => {
    ...
    try {
      ...
      blockContract.reconcile(blocksResponse, (warning) =>
        logger.warn?.(`[job-parser] ${JSON.stringify(warning)}`)
      );
      normalizeEmptyBlocks(blocksResponse, blockContract.blocks, (warning) =>
        logger.warn?.(`[job-parser] ${JSON.stringify(warning)}`)
      );
      if (debug)
        logger.debug?.(
      ...
      if (feedback.length) {
        const failure = providerError(
          "GEMINI_SCHEMA_ERROR",
          `Gemini output failed intermediate schema validation: Block validation failed: ${JSON.stringify(feedback)}`
        );
      ...
      const compactDetail =
        detail.length > 2400 ? `${detail.slice(0, 2400)}…` : detail;
      logger.error?.(
        `[job-parser] Gemini output failed validation.${debug ? ` ${compactDetail}` : ""}`
      );
      const failure = providerError(
        "GEMINI_SCHEMA_ERROR",
        `Gemini output failed intermediate schema validation: ${compactDetail}`,
        error
      );
    ...
    logger.info?.(
      `[job-parser] Gemini structured extraction validated (${extraction.items.length} items).`
    );
...
export function createGeminiProvider(options = {}) {
  ...
  return async (input) => {
    ...
    for (let offset = 0; offset < ids.length; offset += batchSize) {
      ...
      const targetSections = input.sections
        .map((section) => ({
          ...section,
      ...
      logger.info?.(
        `[job-parser] Gemini batch ${batch}/${total} started (${targetIds.length} target blocks).`
      );
      let raw;
      ...
      try {
        ...
        for (
          ...
          try {
            ...
            if (targeted) {
              ...
              logger.warn?.(
                `[job-parser] Gemini batch ${batch}/${total} has ${invalidIds.length} invalid block(s); requesting targeted correction.`
              );
            }
            ...
            if (!targeted)
              logger.warn?.(
                `[job-parser] Gemini batch ${batch}/${total} failed validation; requesting correction ${correctionAttempt + 1}/${maxCorrections}.`
              );
          }
        }
        ...
        throw providerError(
          ...
          `Batch ${batch}/${total}: ${error.message}`,
          error
        );
      }
    }
