...
const descriptors = {
  gemini: {
    ...
    resolveOptions(config, env) {
      ...
      return {
        ...configured,
        decisionMode: env.JOB_PARSER_GEMINI_DECISIONS === "1" || configured.decisionMode,
  ...
  ollama: {
    ...
    resolveOptions(config, env) {
      ...
      return {
        ...configured,
        decisionMode: env.JOB_PARSER_OLLAMA_DECISIONS === "1" || configured.decisionMode,
        model:
        ...
        contextSize:
          numericOverride(env, "JOB_PARSER_OLLAMA_CONTEXT_SIZE") ??
          configured.contextSize,
        maxPromptTokens:
          numericOverride(env, "JOB_PARSER_OLLAMA_MAX_PROMPT_TOKENS") ??
          configured.maxPromptTokens,
        responseTokenReserve:
          numericOverride(env, "JOB_PARSER_OLLAMA_RESPONSE_TOKEN_RESERVE", {
            allowZero: true,
          }) ?? configured.responseTokenReserve,
        maxAttempts:
