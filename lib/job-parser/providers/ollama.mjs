  ...
  properties: {
    decisions: {
      ...
      items: {
        ...
        properties: {
          unitId: { type: "string" },
          action: { enum: ["exclude", "unresolved", "requirement", "responsibility", "alternative", "metadata"] },
          values: { type: "array", items: { type: "string" } },
          metadataKey: { enum: ["company", "title", "location", "workArrangement", "employmentType", "sourceUrl"] },
          value: { type: "string" },
          reason: { type: "string" },
        },
      },
...
function decisionPrompt(blocks) {
  return [
    ...
    "Actions: exclude for context; unresolved when candidate qualification versus context cannot be determined (include a concise reason); requirement for a source-backed candidate qualification. Under an explicit required/preferred heading the application preserves that strength; in neutral or Activities prose it preserves a generic qualification without inventing required/preferred strength. responsibility is only for duties; alternative is only for a clear explicit OR/OU choice and includes exact option values; metadata is only for an explicit metadata value and includes metadataKey plus its exact source value.",
  ...
  return async (input) => {
    if (decisionMode) {
      ...
      for (let offset = 0; offset < blocks.length; offset += batchSize) {
        const target = blocks.slice(offset, offset + batchSize);
        let response;
        let lastError;
        stats.started += 1;
        for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
          try {
            response = await withTimeout(
              (signal) => send({
                model,
                messages: [{ role: "user", content: decisionPrompt(target) }],
            ...
            if (lastError?.code === "OLLAMA_TIMEOUT") stats.timeouts += 1;
            if (["OLLAMA_MODEL_NOT_FOUND", "OLLAMA_AUTH_ERROR", "OLLAMA_CONFIG_ERROR"].includes(lastError?.code)) break;
        ...
        decisions.push(...validateDecisionResponse(decoded.value, target).decisions);
        stats.completed += 1;
      }
