...
import { splitPlannedBatch } from "./batch-planner.mjs";
...
export function createOllamaProvider({
  ...
  return async (input) => {
    ...
    while (pending.length) {
      ...
      let accepted;
      try {
        accepted = await runCorrectionLoop({
          maxCorrections,
          ...
          onCorrection: (attempt) => {
            logger.warn?.(
              `[job-parser] Ollama batch failed validation; requesting correction ${attempt}/${maxCorrections}.`
            );
          },
        });
      } catch (error) {
