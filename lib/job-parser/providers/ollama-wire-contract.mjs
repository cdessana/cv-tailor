  ...
  properties: {
    blocks: {
      ...
      items: {
        ...
        properties: {
          id: { type: "string" },
          status: { enum: ["extracted", "excluded", "unresolved"] },
          reason: { type: "string" },
          records: {
            type: "array",
            items: {
...
export function translateOllamaWireResponse(response, blocks) {
  ...
  for (const result of response.blocks) {
    ...
    if (translated.blocks[result.id])
      invalid(`duplicate block ID ${result.id}.`);
    if (!Array.isArray(result.records))
      invalid(`records for ${result.id} must be an array.`);
    const block = expected.get(result.id);
    const canonical = {
    ...
    for (const record of records) {
      ...
      const sourceSection = block.heading
        ? { sourceSection: block.heading }
        : {};
    ...
    translated.blocks[result.id] = canonical;
  }
  const missing = [...expected.keys()].filter((id) => !translated.blocks[id]);
  if (missing.length) invalid(`missing block IDs: ${missing.join(", ")}.`);
  return translated;
}
