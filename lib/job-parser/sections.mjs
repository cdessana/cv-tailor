...
export function buildSections(lines, originalText) {
  ...
  for (const line of lines) {
    ...
    // Browser textareas and scraped pages frequently lose bullet markers while
    // retaining one logical requirement or task per physical line. Candidate
    // sections preserve those source-line boundaries so consecutive
    // qualifications are not collapsed into a single record.
    const standaloneCandidateLine =
      section.signal === "responsibilities"
        ? /^(?:design|develop|maintain|collaborate|champion|participate|stay|build|lead|drive|create|implement|work)\b/iu.test(line.text)
        : ["required", "preferred"].includes(section.signal) &&
          /^(?:bonus:\s+|bachelor[’']?s?\s+degree\b|master[’']?s?\s+degree\b|ph\.?d\b|\d+\+?\s+years?\b|(?:deep|strong)\s+(?:understanding|grasp|proficiency)\b|hands-on\s+experience\b|comfortable\s+working\b|excellent\s+(?:problem-solving|analytical)\b|(?:proven\s+)?experience\b|knowledge\s+of\b)/iu.test(line.text);
    if (!line.bullet && unit?.type !== "bullet" && standaloneCandidateLine) finishUnit();
    if (!unit) {
      unit = {
