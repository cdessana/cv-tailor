...
function splitLongParagraph(unit, originalText, maximum = 650) {
  ...
  for (let index = 0; index < source.length; index += 1) {
    ...
    if (depth === 0 && /[.!?]/u.test(source[index])
      && (index === source.length - 1 || /\s/u.test(source[index + 1] ?? ""))) boundaries.push(index + 1);
  }
  ...
  return ranges.map(([start, end]) => {
    ...
    return {
      ...
      text: originalText.slice(absoluteStart, absoluteEnd).replace(/\s+/gu, " ").trim(),
    };
  });
}
