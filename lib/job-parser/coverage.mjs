...
export function validateCoverage(document, extraction, { required = false } = {}) {
  ...
  for (const entry of extraction.coverage) {
    ...
    // Unresolved is an explicit accounting decision: the provider made no
    // semantic assertion about this real source unit. It is useful diagnostic
    // information, not broken source accounting.
    if (entry.status === "unresolved") warn("unresolved_unit", entry.reason, { unitId: entry.unitId });
...
export function deriveCoverage(document, extraction) {
  ...
  const validation = !errors.length
    ? validateCoverage(document, result, { required: true })
    : { errors: [], warnings: [] };
  if (!errors.length) errors.push(...validation.errors);
  return { valid: !errors.length, errors, warnings: validation.warnings, extraction: result };
}
