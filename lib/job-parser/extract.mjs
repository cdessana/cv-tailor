...
function metadataFromUnit(unit) {
  ...
  // A standalone, explicitly labeled company or title is source metadata even
  // outside a multi-line archive header. Keep the label requirement narrow so
  // ordinary prose is not promoted to identification metadata.
  if (Object.keys(archiveMetadata).length >= 2) return archiveMetadata;
  if (Object.keys(archiveMetadata).length === 1) {
    const [key] = Object.keys(archiveMetadata);
    if (["company", "title"].includes(key)) return archiveMetadata;
  }
  const match =
  ...
  if (
    ...
    return {
      company: { value: about[2].trim(), evidence: { quote: about[1] }, sourceUnitIds: [unit.id] },
    };
  }
  ...
  return {
    company: { value: company, evidence: { quote: unit.originalText }, sourceUnitIds: [unit.id] },
    title: { value: title, evidence: { quote: unit.originalText }, sourceUnitIds: [unit.id] },
  };
}

function metadataFromJoinAsLine(unit) {
  const match = unit.originalText.match(
    /(?:^|\r?\n)Join\s+([\p{L}\p{N}][\p{L}\p{N} .&'-]{1,80}?)\s+as\s+(?:an?\s+)?([\p{L}\p{N}][\p{L}\p{N} .&'-]{1,100}?)(?:\s+and\s+(?:help|shape|build|work|make)\b|[.!]|$)/ium
  );
  if (!match) return null;
  const company = match[1].trim();
...
function archiveBoilerplateReason(document, unit) {
  ...
  if (/^(?:Location|Date Posted|Official Job ID|Job ID|Direct LinkedIn Link):\s*/iu.test(text))
    return "Archive detail field repeats structured header metadata.";
  if (
...
function withNestedDetails(item, details) {
  if (!details?.length || item.type !== "item") return item;
  const value = item.value
    .replace(/\s*(?:including|incluindo|using|usando)\s*:\s*$/iu, "")
    .replace(/,\s*$/u, "")
    .trim();
  return {
    ...item,
    ...(value ? { value } : {}),
    examples: details.map((unit) => ({ value: unit.text.trim() })),
  };
}
...
function isQualificationProse(value) {
  return /^(?:bonus:\s+|bachelor[’']?s?\s+degree\b|master[’']?s?\s+degree\b|ph\.?d\b|\d+\+?\s+years?\b|(?:deep|strong)\s+(?:understanding|grasp|proficiency)\b|hands-on\s+experience\b|comfortable\s+working\b|excellent\s+(?:problem-solving|analytical)\b|(?:proven\s+)?experience\b|knowledge\s+of\b)/iu.test(value);
}

function isResponsibilityProse(value) {
  return /^(?:design|develop|maintain|collaborate|champion|participate|stay|build|lead|drive|create|implement|work)\b/iu.test(value);
}

function signaledFallback(section, unit) {
  const value = unit.text.trim();
  if (
    !value ||
    ...
    return null;
  // Headings are decisive for bullets. Paragraphs need an additional narrow,
  // source-visible cue so role/team context under candidate headings does not
  // become an invented requirement or responsibility.
  if (unit.type !== "bullet") {
    if (
      ["required", "preferred"].includes(section.signal) &&
      !isQualificationProse(value)
    ) return null;
    if (
      section.signal === "responsibilities" &&
      !isResponsibilityProse(value)
    ) return null;
    if (
      !["required", "preferred", "responsibilities"].includes(section.signal)
    ) return null;
  }
  let candidate;
  if (section.signal === "responsibilities") {
    ...
    // Preserve complete bullets, including expressions whose alternatives are
    // not yet safe to canonicalize.
    const kind = section.signal === "competencies" ? "competency" : "requirement";
    const shared = {
      kind,
      classification:
        section.signal === "required" && /^bonus:\s+/iu.test(value)
          ? "preferred"
          : section.signal === "competencies"
            ? "ambiguous"
            : section.signal,
      evidence: { quote: unit.originalText },
      sourceUnitIds: [unit.id],
      ...(section.heading?.text.replace(/\s+/gu, " ").trim().toLocaleLowerCase() === "requirements added by the job poster"
        ? { provenance: "job-poster-added" }
        : {}),
      ...(section.heading ? { sourceSection: section.heading.text } : {}),
    };
    candidate = { type: "item", value, ...shared };
  }
...
export function extract(document, dictionary = defaultAliases) {
  ...
  for (const section of document.sections) {
    const nestedDetailsByParent = new Map();
    for (let unitIndex = 0; unitIndex < section.units.length; unitIndex += 1) {
      const unit = section.units[unitIndex];
      if (unit.type !== "bullet") continue;
      const parent = section.units
        .slice(0, unitIndex)
        .reverse()
        .find((candidate) =>
          candidate.type === "bullet" &&
          candidate.indentation.length < unit.indentation.length
        );
      if (
        parent &&
        /\b(?:including|incluindo|using|usando)\s*:\s*$/iu.test(parent.text)
      ) {
        const details = nestedDetailsByParent.get(parent.id) ?? [];
        details.push(unit);
        nestedDetailsByParent.set(parent.id, details);
      }
    }
    const nestedDetailIds = new Set(
      [...nestedDetailsByParent.values()].flatMap((details) => details.map((unit) => unit.id))
    );
    ...
    for (const unit of section.units) {
      if (nestedDetailIds.has(unit.id)) {
        coverage.push({
          unitId: unit.id,
          status: "excluded",
          reason: "Indented technology detail is preserved as an example on its broader source item and is not an independent requirement.",
        });
        continue;
      }
      ...
      // Identification patterns are independent of content sections: vacancy
      // pages often repeat a title-bearing call to action after Requirements.
      const detectedMetadata = metadataFromUnit(unit) ?? metadataFromJoinAsLine(unit);
      if (detectedMetadata) {
        for (const [key, value] of Object.entries(detectedMetadata)) {
          // A labeled Company header is more direct than the company wording
          // repeated in a closing Join-as call to action.
          if (key === "company" && metadata.company) continue;
          metadata[key] = metadata[key]
            ? mergeMetadataRecord(metadata[key], value, key)
            : value;
        }
        coverage.push({
          unitId: unit.id,
          status: "metadata",
          metadataKeys: Object.keys(detectedMetadata),
        });
        continue;
      }
      ...
      const fallback = result.reason ? signaledFallback(section, unit) : null;
      if (fallback) {
        items.push(withNestedDetails(fallback, nestedDetailsByParent.get(unit.id)));
        continue;
      }
      if (result.reason) {
        unresolved.push({
      ...
      items.push(withNestedDetails({
        ...
        sourceUnitIds: [unit.id],
        ...(section.heading ? { sourceSection: section.heading.text } : {}),
      }, nestedDetailsByParent.get(unit.id)));
    }
  }
  const extraction = {
