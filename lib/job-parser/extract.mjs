  ...
  for (const section of document.sections) {
    const nestedDetailIds = new Set(
      section.units.flatMap((unit, unitIndex) => {
        const parent = section.units[unitIndex - 1];
        return unit.type === "bullet" &&
          parent?.type === "bullet" &&
          unit.indentation.length > parent.indentation.length &&
    ...
    for (const unit of section.units) {
      if (nestedDetailIds.has(unit.id)) {
        coverage.push({
          unitId: unit.id,
          status: "excluded",
          reason: "Indented technology detail remains attached to its broader source item and is not an independent requirement.",
        });
        continue;
      }
