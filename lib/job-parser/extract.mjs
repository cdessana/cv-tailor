...
function metadataFromUnit(unit) {
  ...
  const about = text.match(/(?:^|\s)(Sobre o ([\p{L}\p{N}][\p{L}\p{N} .&'-]*?))(?=\s|$)/iu);
  if (!match && about && !/^(?:área|empresa|time|você)$/iu.test(about[2].trim())) {
    return { company: { value: about[2].trim(), evidence: { quote: about[1] } } };
  }
