...
function descriptor(unit, section, action) {
  ...
  if (action === "requirement") {
    // Explicit headings establish strength. For genuinely ambiguous prose,
    // retain a generic requirement rather than inventing required/preferred
    // strength or discarding a source-backed qualification.
    if (section.role === "context") return null;
    return {
      kind: "requirement",
      classification: ["required", "preferred"].includes(section.signal)
        ? section.signal
        : "ambiguous",
    };
  }
...
export function materializeSemanticDecisions(document, response) {
  ...
  for (const decision of response.decisions) {
    ...
    if (decision.action === "unresolved") {
      if (typeof decision.reason !== "string" || !normalized(decision.reason))
        throw new TypeError("Semantic unresolved decision requires a reason.");
      coverage.push({
