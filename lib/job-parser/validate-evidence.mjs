...
export function validateEvidence(document, extraction, dictionary = defaultAliases) {
  ...
  for (const [key, metadata] of Object.entries(extraction.metadata ?? {})) {
    ...
    if (key === "employmentType" && /^(?:100%\s*)?(?:remote(?:\s+work)?|hybrid|on[ -]?site|remoto|remota|híbrido|híbrida|presencial)$/iu.test(metadata.value.trim())) {
      errors.push(issue("invalid_employment_type", `${path}/value`, "Work arrangement is not an employment type.", { value: metadata.value }));
    }
    ...
    if (!valueSupported(metadata.value, quote, dictionary, index)) {
      errors.push(issue("value_not_supported_by_evidence", `${path}/value`,
        `Extracted ${key} is not supported by its source evidence.`, { value: metadata.value, quote }));
    }
  }
