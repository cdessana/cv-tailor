...
function mergeMetadata(deterministic, semantic) {
  ...
  for (const [key, value] of Object.entries(semantic.metadata ?? {})) {
    merged[key] = merged[key] ? mergeMetadataRecord(merged[key], value, key) : structuredClone(value);
  }
...
export async function semanticExtract(document, deterministic, provider, { onResponse } = {}) {
  ...
  const semantic = canonicalizeDirectAlternatives(response);
  ...
  for (const item of semantic.items) {
    ...
    if (index < 0) {
      ...
      const kept = structuredClone(quote.length > semanticText(existing.evidence.quote).length ? item : existing);
      ...
      if (existing.examples || item.examples) {
        kept.examples = [...new Map([...(existing.examples ?? []), ...(item.examples ?? [])]
          .map(example => [semanticText(example.value), structuredClone(example)])).values()];
      }
