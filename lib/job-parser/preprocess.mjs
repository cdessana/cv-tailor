...
function scanLines(text) {
  ...
  for (const match of text.matchAll(pattern)) {
    ...
    // Some archive exports embed a recognized heading and its first bullet at
    // the end of an introductory sentence on the same physical line.
    const embedded = content.match(/^(.*?)(No seu dia a dia, você vai:|Obrigatório|Diferenciais)\s*(•|◦|▪|\d+\.(?!\d)|\d+\))\s*(.+)$/iu);
    if (embedded && embedded[1].trim()) {
      const prefix = embedded[1];
      const headingText = embedded[2].trim();
      const bulletChar = embedded[3];
      const bulletText = embedded[4].trim();
      const headingOffset = content.indexOf(embedded[2], prefix.length);
      const markerOffset = content.indexOf(bulletChar, headingOffset + headingText.length);
      const prefixEnd = match.index + indentation.length + headingOffset;
      const headingStart = prefixEnd;
