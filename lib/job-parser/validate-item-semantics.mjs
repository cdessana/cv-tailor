...
const examplePattern = /\(\s*(?:such as\b|e\.g\.|for example\b|como\b|por exemplo\b)[^()]*\)/giu;
const trailingIllustration = /(?:,\s*)?\b(?:like|such as|e\.g\.|for example|como|por exemplo)\b[\s\S]*$/iu;
...
export function validateItemSemantics(item, { metadata = {} } = {}) {
  ...
  if (item.type === "alternative") {
    ...
    if (qualification && choice.test(remaining) && !isDescriptiveOr(remaining)) {
