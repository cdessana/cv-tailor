  ...
  for (const [index, work] of (resume.work ?? []).entries()) {
    ...
    for (const [factIndex, fact] of facts.entries()) {
      ...
      claims.push({
        ...
        skills: explicitSkills.map((skill) => skill.name), source: source("json_resume", reference), reviewStatus: "pending", createdAt, updatedAt: createdAt,
      });
...
export function applyQuestionnaireAnswers(candidate, answers) {
  ...
  for (const answer of answers ?? []) {
    ...
    if (!next.claims.some((item) => item.id === claimId)) next.claims.push({ id: claimId, contextId: question.contextId, claim: text, originalClaim: text, normalizedClaim: normalizedClaim(text), skills: [], source: source("questionnaire", question.id), reviewStatus: "pending", createdAt: updatedAt, updatedAt });
  }
...
export function applyReviewDecisions(candidate, decisions) {
  ...
  for (const decision of decisions ?? []) {
    ...
    next.reviewDecisions.push({ id: claim.reviewDecision.id, claimId: claim.id, contextId: claim.contextId, status: decision.status, note: claim.reviewDecision.note, actor: claim.reviewDecision.actor, decidedAt, sources: [claim.source] });
  }
  next.updatedAt = updatedAt;
  assertEvidenceCandidate(next);
  return { candidate: next, report: createReport(next) };
