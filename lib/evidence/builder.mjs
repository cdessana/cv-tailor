...
export function applyReviewDecisions(candidate, decisions) {
  const next = structuredClone(candidate);
  const updatedAt = now();
  const reviewText = (value, field) => {
    const text = [...String(value ?? "")].filter((character) => { const code = character.charCodeAt(0); return code >= 32 && code !== 127; }).join("").trim();
    if (text.length > 1000) throw new EvidenceBuilderError("EVIDENCE_REVIEW_FIELD_INVALID", `${field} is too long.`);
    return text;
  };
  for (const decision of decisions ?? []) {
    if (decision.issueId) {
      ...
      issue.resolved = true;
      const note = reviewText(decision.note, "note");
      const actor = reviewText(decision.actor ?? "user", "actor");
      issue.resolution = { values, note, decidedAt, actor };
      next.reviewDecisions.push({ id: stableId("review", issue.id, "resolved", values[0], decidedAt), issueId: issue.id, contextId: issue.contextId, status: "resolved", values, note, actor, decidedAt, sources: issue.sources });
      continue;
    }
    ...
    const note = reviewText(decision.note, "note");
    const actor = reviewText(decision.actor ?? "user", "actor");
    claim.reviewDecision = { id: stableId("review", claim.id, decision.status, note, decidedAt), status: decision.status, note, actor, decidedAt };
