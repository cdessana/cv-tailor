  ...
  for (const supportingSource of supportingSources) {
    ...
    for (const [index, input] of supportingSource.claims.entries()) {
      ...
      const claim = {
        ...
        originalClaim: claimText, normalizedClaim: normalizedClaim(claimText), skills: Array.isArray(input.skills) ? input.skills.map((skill) => normalizeSafeTerminology(skill)).filter(Boolean) : [],
...
function addClaims({ context, facts, sourcePrefix, summaryField, hasSummary, skills, claims, issues, questions, createdAt }) {
  for (const [factIndex, fact] of facts.entries()) {
    ...
    addAmbiguity({ contextId: context.id, claim: fact, claimSource: source("json_resume", reference), issues, questions, createdAt });
  }
}
...
export function createReport(candidate) {
  ...
  const claimReviews = claims.map((claim) => ({
    ...
    allowedActions: claim.reviewStatus === "pending" ? ["approve", "reject"] : claim.reviewStatus === "conflict" ? ["resolve_conflict"] : [],
  }));
  ...
  for (const [index, input] of inputProjects.entries()) {
    ...
    for (const [factIndex, fact] of (input.facts ?? []).entries()) {
      ...
      if (!next.claims.some((item) => item.id === claimId)) {
        ...
        next.claims.push({ id: claimId, contextId: context.id, claim, originalClaim: claim, normalizedClaim: normalizedClaim(claim), skills, source: claimSource, sources: [claimSource], reviewStatus: "pending", createdAt: updatedAt, updatedAt });
      }
    }
...
export function applyQuestionnaireAnswers(candidate, answers) {
  ...
  for (const answer of answers ?? []) {
    ...
    addAmbiguity({ contextId: question.contextId, claim: text, claimSource: source("questionnaire", question.id), issues: next.issues, questions: next.questionnaire.questions, createdAt: updatedAt });
  }
