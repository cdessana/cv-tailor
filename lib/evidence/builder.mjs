...
function addClaims({ context, facts, sourcePrefix, summaryField, hasSummary, skills, claims, issues, questions, createdAt }) {
  for (const [factIndex, fact] of facts.entries()) {
    ...
    if (ambiguity) {
      issues.push({ id: stableId("ambiguity", context.id, reference, fact), type: "ambiguous_technology", contextId: context.id, values: [fact.trim()], sources: [source("json_resume", reference)], question: ambiguity.prompt, resolved: false, createdAt });
