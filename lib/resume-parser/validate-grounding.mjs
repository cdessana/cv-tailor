...
export function inspectResumeGrounding({ resume, provenance, document }) {
  const issues = [];
  const fields = scalarFields(resume);
  ...
  const invalidPaths = new Set(issues.map(({ path }) => path).filter(Boolean));
  return {
    issues,
    summary: {
      valuesChecked: fields.length,
      provenanceRecords: provenance.length,
      groundedValues: Math.max(0, fields.length - invalidPaths.size),
      errors: issues.length,
    },
  };
}

export function validateResumeGrounding(input) {
  return inspectResumeGrounding(input).issues;
}
