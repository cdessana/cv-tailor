...
function isBullet(line) {
  return typeof line === "object" && line?.kind === "bullet"
    || /^\s*(?:[-*•]\s+|\d+[.)]\s+)/u.test(textOf(line));
}

function highlightText(line) {
  return textOf(line).replace(/^\s*(?:[-*•]\s+|\d+[.)]\s+)/u, "").trim();
}
...
function expandInlineIdentity(input) {
  if (isBullet(input)) return [input];
  ...
  for (const input of lines.flatMap(expandInlineIdentity)) {
    ...
    if (isBullet(input)) {
      if (work.length && pending.length === 0) {
        work.at(-1).value.highlights.push(highlightText(input));
    ...
    if (lineDateResult.value && pending.length >= 2) {
      ...
      pending.push(input);
    } else if (work.length && pending.length === 0) {
      work.at(-1).value.highlights.push(highlightText(input));
      work.at(-1).sources.highlights.push(source);
    } else pending.push(input);
  }
