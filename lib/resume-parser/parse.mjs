...
import { assessResumeContent } from "./content-assessment.mjs";
import { inspectResumeGrounding } from "./validate-grounding.mjs";

const sectionNames = new Map([
  ["experience", "work"], ["work experience", "work"], ["professional experience", "work"],
  ["experiência", "work"], ["experiencia", "work"], ["experiência profissional", "work"], ["experiencia profissional", "work"],
  ["education", "education"], ["educação", "education"], ["educacao", "education"], ["formação", "education"], ["formacao", "education"], ["formação acadêmica", "education"], ["formacao academica", "education"],
  ["skills", "skills"], ["technical skills", "skills"], ["habilidades", "skills"], ["competências técnicas", "skills"], ["competencias tecnicas", "skills"],
  ["certificates", "certificates"], ["certifications", "certificates"],
  ["certificados", "certificates"], ["certificações", "certificates"], ["certificacoes", "certificates"],
...
const bulletPrefix = /^\s*(?:[-*•]\s+|\d+[.)]\s+)/u;
const standaloneBullet = /^\s*(?:[-*•]|\d+[.)])\s*$/u;
...
function splitSections(text, sourceLines = null) {
  ...
  for (const rawLine of (sourceLines ?? text.split("\n").map((value) => ({ text: value })))) {
    const rawText = rawLine.text;
    if (standaloneBullet.test(rawText)) {
      nextLineIsBullet = true;
      continue;
    }
    const line = clean(rawLine.text);
    if (!line) continue;
    const heading = line.toLowerCase().replace(/\s*[:：]\s*$/u, "");
    if (sectionNames.has(heading)) {
      current = sectionNames.get(heading);
      sections[current] ??= [];
      nextLineIsBullet = false;
      continue;
    }
    sections[current] ??= [];
    sections[current].push({
      text: line,
      rawText,
      kind: nextLineIsBullet || bulletPrefix.test(rawText) ? "bullet" : "text",
      source: rawLine.source ?? null,
    });
    nextLineIsBullet = false;
  }
  return sections;
...
function addIssue(issues, code, message, sourceText, extras) {
  issues.push(reviewIssue(code, message, sourceText, extras));
}

export function parseResumeText(text, { format = "txt", lines = null } = {}) {
  const issues = [];
  const document = lines ? { text, lines } : documentFromText(text, { format });
  const sections = splitSections(document.text, document.lines);
  ...
  const extractedCertificates = extractCertificateEntries(sections.certificates ?? [], (code, message, sourceText) => addIssue(issues, code, message, sourceText));
  ...
  const contentAssessment = assessResumeContent({ resume, sections });
  if (!contentAssessment.valid) {
    addIssue(
      issues,
      "malformed_resume_content",
      "The source does not contain the minimum recognizable resume structure: a candidate name and at least one independent resume signal.",
      text,
      { severity: "error", requiresHumanReview: false, details: contentAssessment }
    );
  }
  const provenance = [
  ...
  const grounding = inspectResumeGrounding({ resume, provenance, document });
