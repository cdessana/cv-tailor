...
function parseBasicsEntries(lines) {
  ...
  const urls = lines.flatMap((line) => (typeof line === "string" ? line : line.text)
    .match(/https?:\/\/[^\s|]+/gu)
    ?.map((url) => ({ url, source: typeof line === "string" ? null : line.source })) ?? []);
  ...
  if (email) { basics.email = email.match(/[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}/u)[0]; sources.email = lines[values.indexOf(email)]?.source ?? null; }
  if (phone) { basics.phone = phone.match(/\+?\d[\d ()-]{7,}\d/u)[0]; sources.phone = lines[values.indexOf(phone)]?.source ?? null; }
  if (urls.length) {
    basics.profiles = urls.map(({ url }) => ({ network: /linkedin/iu.test(url) ? "LinkedIn" : "Website", url }));
    sources.profiles = urls.map(({ source }) => source);
  }
