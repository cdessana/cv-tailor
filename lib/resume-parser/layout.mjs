...
function columnGapThreshold(words) {
  const characterWidths = words
    .filter(({ text, xMin, xMax }) => text.length && xMax > xMin)
    .map(({ text, xMin, xMax }) => (xMax - xMin) / [...text].length);
  if (!characterWidths.length) return 42;
  return Math.max(18, Math.min(72, median(characterWidths) * 7));
}

function splitRowIntoSegments(words, gapThreshold) {
  const sorted = [...words].sort((left, right) => left.xMin - right.xMin);
  const segments = [];
  let segment = [];
  for (const word of sorted) {
    const previous = segment.at(-1);
    if (previous && word.xMin - previous.xMax > gapThreshold) {
...
function pageLines(words, page) {
  const rows = [];
  const gapThreshold = columnGapThreshold(words);
  ...
  const segments = rows.flatMap((row) => splitRowIntoSegments(row.words, gapThreshold).map((items) => ({
    y: row.y,
    x: items[0].xMin,
    items,
