...
(function () {
  ...
  function renderEvidenceBuilder(data) {
    ...
    container.innerHTML = `
      ...
      <div class="space-y-2 max-h-72 overflow-y-auto">${claims.map((claim) => { const context = (data.candidate.contexts || []).find((item) => item.id === claim.contextId); return `<div class="border border-slate-200 rounded-lg p-3 text-xs"><div class="flex justify-between gap-2"><span class="font-semibold text-slate-900">${escapeHtml(claim.claim)}</span><span class="text-[10px] uppercase font-bold">${escapeHtml(claim.reviewStatus)}</span></div><p class="text-slate-600 mt-1">${escapeHtml(context ? `${context.company} · ${context.position} · ${context.period}` : claim.contextId)}</p><p class="text-slate-500 mt-1">Source: ${escapeHtml(claim.source.type)}:${escapeHtml(claim.source.reference)}</p>${claim.reviewStatus === "pending" ? `<div class="mt-2 flex gap-2"><button data-claim="${escapeHtml(claim.id)}" data-status="approved" class="btn-review-claim text-[11px] font-bold text-emerald-700">Approve</button><button data-claim="${escapeHtml(claim.id)}" data-status="rejected" class="btn-review-claim text-[11px] font-bold text-slate-600">Reject</button></div>` : ""}</div>`; }).join("")}</div>`;
